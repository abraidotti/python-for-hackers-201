...
def hook_function(nCode, wParam, lParam):
    ...
    if wParam == WM_KEYDOWN:
        ...
        buffer = (c_ushort * 1)()
        n = ToAscii(keyboard.vkCode, keyboard.scanCode, state, buffer, 0)
        ...
        if n > 0:
            if keyboard.vkCode == WM_RETURN:
                ...
                print(
                    "{}".format(string_at(buffer).decode("latin-1")), end="", flush=True
                )
...
# run script and type something here in the editor; check the command line
